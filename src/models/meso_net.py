...
class MesoInception4(nn.Module):
    ...
    def _compute_embedding(self, input):
        ...
        x = x.view(x.size(0), -1)        # Flatten (Batch, 16*8*8)
        x = self.dropout(x)
...
class FrontendMesoInception4(MesoInception4):
    ...
    def forward(self, x):
        x = self.frontend(x)
        if x.dim() == 5:                # Check if output from frontend is 5D
            x = x.mean(dim=-1)          # Reduce last dimension if 5D (e.g., averaging)
        x = self._compute_embedding(x)
        return x

if __name__ == "__main__":
    model = FrontendMesoInception4(
        input_channels=1,  # Adjust based on frontend output
        fc1_dim=1024,
        device='cuda',
