...
class FrontendMesoInception4(MesoInception4):
    ...
    def forward(self, x):
        ...
        # If output from frontend has extra dimensions, reduce it to 4D
        if x.ndim == 5:  # Assuming shape is (batch, channels, height, width, extra_dim)
            x = x.mean(-1)  # Average or select along the last dimension
        elif x.ndim > 4:
            raise ValueError(f"Unexpected number of dimensions from frontend: {x.ndim}")
